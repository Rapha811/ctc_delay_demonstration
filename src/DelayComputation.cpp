...
bool build_cn_and_contract_delay(IntervalVector& delay_in_out, IntervalVector& tdoa, const Tube& y_, const Tube& e_){
    ...
    //WLOG: one signal MUST arrive before the other or they arrive at the same time
...
void compute_delay_subpaving(IntervalVector& delay_in_out, const Tube& y_, const Tube& e_, float precision,
                                       vector<IntervalVector>& subpaving_delay, vector<IntervalVector>& subpaving_tdoa, IntervalVector& tdoa_out){
